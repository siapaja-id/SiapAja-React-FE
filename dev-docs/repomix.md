...
## File: src/constants/domain.constant.tsx
```typescript
import React from 'react';
...
export const SocialPost: React.FC<FeedItemProps> = ({ data, onClick, onUserClick, isMain, isParent, hasLineBelow, canAcceptBid, onAcceptBid, isQuote }) => {
  ...
  return (
    <BaseFeedCard 
      ...
      isQuote={isQuote}
      headerMeta={
        spData.threadCount && spData.threadCount > 1 ? (
          <span className="ml-1.5 px-1.5 py-0.5 rounded-md bg-primary/10 text-primary border border-primary/20 text-[9px] font-black tracking-widest flex items-center shadow-sm">
