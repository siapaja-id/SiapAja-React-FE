...
<header className="sticky top-0 z-50 bg-header-bg border-b border-on-surface/5 will-change-transform mb-4">
          <div className="flex justify-between items-center px-4 h-16">
            ...
            <div className="flex space-x-6">
              {['for-you', 'around-you'].map(tab => (
                <button key={tab} onClick={() => setColumnActiveTab(columnId, tab as TabState)} className={`text-sm font-semibold relative py-1 capitalize ${activeTab === tab ? 'text-on-header' : 'text-on-header-secondary'}`}>
                  {tab.replace('-', ' ')}
                  {activeTab === tab && <motion.div layoutId={`tab-${columnId}`} className="absolute -bottom-1 left-0 right-0 h-0.5 bg-primary" />}
                </button>
              ))}
            </div>
            <button onClick={() => isDesktop ? openColumn('/profile', columnId) : navigate('/profile')} className="flex items-center gap-1 bg-black/20 border border-white/10 rounded-full px-3 py-1 hover:bg-white/10 transition-colors shadow-sm">
