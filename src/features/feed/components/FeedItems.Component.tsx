...
import { useFeedCard, useSocialPost } from '@/src/features/feed/hooks/useFeedCard';
...
export const BaseFeedCard: React.FC<BaseFeedCardProps> = ({ data, onClick: onClickOverride, avatarContent, headerMeta, children }) => {
  ...
  const { resolvedIsAuthor, handleCardClick, handleUserClick } = useFeedCard(data, onClickOverride, isQuote, isParent);
...
export const SocialPost: React.FC<SocialPostProps> = ({ data, onClick }) => {
  ...
  const { canAcceptBid, handleAcceptBid } = useSocialPost(spData as SocialPostData, routeId);
